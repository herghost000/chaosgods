...
green.forEach((color, index) => {
  ...
  colors += `  --c-color-success-${index + 1}: ${color};\n`
})

// 警告我们使用金色
gold.forEach((color, index) => {
  // 我们还是使用同样的方式生成，这里我们就直接带着大家去复制了。
  colors += `  --c-color-warning-${index + 1}: ${color};\n`
})

// 失败使用红色
red.forEach((color, index) => {
  // 我们还是使用同样的方式生成，这里我们就直接带着大家去复制了。
  colors += `  --c-color-error-${index + 1}: ${color};\n`
})
...
// console.log(colors)
// 接下来我们就需要写入到我们的文件中，
// 首先我们需要在packages/c-ui/src中创建一个style的文件夹，然后再创建一个themes的文件夹
...
// 我们先创建这个文件的存储路径
const filePath = fileURLToPath(new URL('../packages/c-ui/src/style/themes/colors.less', import.meta.url))
// 先创建这个文件，
