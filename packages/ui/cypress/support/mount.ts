...
import { createChaos } from '../../src/framework'
 ...
 */
Cypress.Commands.add('mount', (component, options, chaosOptions) => {
  const root = document.getElementById('cy-root')!

  // add the v-application class that allows Chaos styles to work
  if (!root.classList.contains('v-locale--is-rtl'))
    root.classList.add('v-locale--is-ltr')

  const chaos = createChaos(mergeDeep({
    icons: { aliases },
  }, chaosOptions))
