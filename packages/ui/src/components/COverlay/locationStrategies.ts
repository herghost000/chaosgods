...
function getIntrinsicSize(el: HTMLElement, isRtl: boolean) {
  ...
  if (isRtl)
    contentBox.x += Number.parseFloat(`${el.style.right || 0}`)
  else
    contentBox.x -= Number.parseFloat(`${el.style.left || 0}`)

  contentBox.y -= Number.parseFloat(`${el.style.top || 0}`)
...
function connectedLocationStrategy(data: LocationStrategyData, props: StrategyProps, contentStyles: Ref<Record<string, string>>) {
  ...
  const [minWidth, minHeight, maxWidth, maxHeight]
    = (['minWidth', 'minHeight', 'maxWidth', 'maxHeight'] as const).map((key) => {
      return computed(() => {
        const val = Number.parseFloat(`${props[key]!}`)
        return Number.isNaN(val) ? Number.POSITIVE_INFINITY : val
      })
    })
  ...
  const offset = computed(() => {
    ...
    if (typeof props.offset === 'string') {
      const offset = props.offset.split(' ').map(Number.parseFloat)
      if (offset.length < 2)
        offset.push(0)
      return offset
    }
  ...
  function updateLocation() {
    ...
    if (!scrollParents.length) {
      ...
      if (!(data.contentEl.value.style.top && data.contentEl.value.style.left)) {
        contentBox.x -= Number.parseFloat(`${document.documentElement.style.getPropertyValue('--v-body-scroll-x') || 0}`)
        contentBox.y -= Number.parseFloat(`${document.documentElement.style.getPropertyValue('--v-body-scroll-y') || 0}`)
      }
    }
    ...
    Object.assign(contentStyles.value, {
      ...
      'maxWidth': convertToUnit(pixelCeil(clamp(available.x, minWidth.value === Number.POSITIVE_INFINITY ? 0 : minWidth.value, maxWidth.value))),
      'maxHeight': convertToUnit(pixelCeil(clamp(available.y, minHeight.value === Number.POSITIVE_INFINITY ? 0 : minHeight.value, maxHeight.value))),
    })
