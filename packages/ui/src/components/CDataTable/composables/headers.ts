...
export const CDataTableHeadersSymbol: InjectionKey<{
  headers: Ref<InternalDataTableHeader[][]>
  columns: Ref<InternalDataTableHeader[]>
}> = Symbol.for('chaos:data-table-headers')
...
function parseFixedColumns(items: InternalDataTableHeader[]) {
  ...
  function setFixed(item: InternalDataTableHeader, parentFixed = false) {
    ...
    if (item.fixed) {
      if (item.children) {
        ...
        if (!seenFixed)
          item.lastFixed = true
        else if (Number.isNaN(+item.width!))
  ...
  function setFixedOffset(item: InternalDataTableHeader, fixedOffset = 0) {
    ...
    if (item.children) {
      ...
      item.fixedOffset = fixedOffset
      fixedOffset += Number.parseFloat(`${item.width}` || '0') || 0
    }
    ...
    while (rowSize > 0) {
      ...
      if (item.children) {
        for (const child of item.children) {
          ...
          const sort = priority % 1 + (fraction / 10 ** (currentDepth + 2))
...
export function createHeaders(
  ...
  provide(CDataTableHeadersSymbol, data)

  return data
}

export function useHeaders() {
  const data = inject(CDataTableHeadersSymbol)
