...
import type { CSliderTrack } from './CSliderTrack'
...
type SliderProvide = {
  ...
  tickSize: Ref<number>
  trackContainerRef: Ref<CSliderTrack | undefined>
...
export const CSliderSymbol: InjectionKey<SliderProvide> = Symbol.for('chaos:v-slider')
...
export function useSteps(props: SliderProps) {
  const min = computed(() => Number.parseFloat(`${props.min}`))
  const max = computed(() => Number.parseFloat(`${props.max}`))
  const step = computed(() => +props.step > 0 ? Number.parseFloat(`${props.step}`) : 0)
  ...
  function roundValue(value: string | number) {
    ...
    return Number.parseFloat(Math.min(newValue, max.value).toFixed(decimals.value))
  }
...
export function useSlider({
  ...
  const thumbSize = computed(() => Number.parseInt(`${props.thumbSize}`, 10))
  const tickSize = computed(() => Number.parseInt(`${props.tickSize}`, 10))
  const trackSize = computed(() => Number.parseInt(`${props.trackSize}`, 10))
  ...
  const trackContainerRef = ref<CSliderTrack | undefined>()
  ...
  const parsedTicks = computed<Tick[]>(() => {
    ...
    if (!props.ticks) {
      return numTicks.value !== Number.POSITIVE_INFINITY
    ...
    if (Array.isArray(props.ticks))
      return props.ticks.map(t => ({ value: t, position: position(t), label: t.toString() }))
    return Object.keys(props.ticks).map(key => ({
      value: Number.parseFloat(key),
      position: position(Number.parseFloat(key)),
