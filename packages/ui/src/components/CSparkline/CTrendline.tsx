...
export const makeCTrendlineProps = propsFactory({
  fill: Boolean,

  ...makeLineProps(),
}, 'CTrendline')

export const CTrendline = genericComponent<CTrendlineSlots>()({
  name: 'CTrendline',

  props: makeCTrendlineProps(),
  ...
  setup(props, { slots }) {
    ...
    const lineWidth = computed(() => {
      return Number.parseFloat(`${props.lineWidth}`) || 4
    })
    ...
    const boundary = computed<Boundary>(() => {
      ...
      return {
        ...
        minY: padding,
        maxY: Number.parseInt(`${props.height}`, 10) - padding,
      }
    })
    ...
    useRender(() => {
      ...
      return (
        <svg
          display="block"
          stroke-width={Number.parseFloat(`${props.lineWidth}`) ?? 4}
        >
          <defs>
            <linearGradient
              id={id.value}
            ...
            >
              {
                parsedLabels.value.map((item, i) => (
                  <text
                    ...
                    y={(Number.parseInt(`${props.height}`, 10) - 4) + (Number.parseInt(`${props.labelSize}`, 10) || 7 * 0.75)}
...
export type CTrendline = InstanceType<typeof CTrendline>
