import './CTimePicker.sass'
import { computed, onMounted, ref, watch } from 'vue'
import type { PropType } from 'vue'
import { CTimePickerClock } from './CTimePickerClock'
import { CTimePickerControls } from './CTimePickerControls'
...
export type CTimePickerSlots = Omit<CPickerSlots, 'header'>

export const makeCTimePickerProps = propsFactory({
...
export const CTimePicker = genericComponent<CTimePickerSlots>()({
  ...
  setup(props, { emit, slots }) {
    ...
    const controlsRef = ref<CTimePickerControls | null>(null)
    const clockRef = ref<CTimePickerClock | null>(null)
    ...
    function setInputData(value: string | null | Date) {
      if (value == null || value === '') {
        ...
        const [hour, , minute, , second, period] = value.trim().toLowerCase().match(/^(\d+):(\d+)(:(\d+))?([ap]m)?$/) || Array.from({ length: 6 })

        inputHour.value = period ? convert12to24(Number.parseInt(hour, 10), period as Period) : Number.parseInt(hour, 10)
        inputMinute.value = Number.parseInt(minute, 10)
        inputSecond.value = Number.parseInt(`${second || 0}`, 10)
      }
    ...
    useRender(() => {
      ...
      const timePickerControlsProps = CTimePickerControls.filterProps(props)
      const timePickerClockProps = CTimePickerClock.filterProps(omit(props, ['format', 'modelValue', 'min', 'max']))
...
export type CTimePicker = InstanceType<typeof CTimePicker>
