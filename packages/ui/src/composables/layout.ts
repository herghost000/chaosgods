...
import type { CSSProperties, ComponentInternalInstance, InjectionKey, Prop, Ref } from 'vue'
...
export const ChaosLayoutKey: InjectionKey<LayoutProvide> = Symbol.for('chaos:layout')
export const ChaosLayoutItemKey: InjectionKey<{ id: string }> = Symbol.for('chaos:layout-item')
...
export function useLayout() {
  const layout = inject(ChaosLayoutKey)

  if (!layout)
    throw new Error('[Chaos] Could not find injected layout')
...
function generateLayers(layout: string[], positions: Map<string, Ref<Position>>, layoutSizes: Map<string, Ref<number | string>>, activeItems: Map<string, Ref<boolean>>): { id: string, layer: Layer }[] {
  ...
  for (const id of layout) {
    ...
    const layer = {
      ...previousLayer,
      [position.value]: Number.parseInt(`${previousLayer[position.value]}`, 10) + (active.value ? Number.parseInt(`${amount.value}`, 10) : 0),
    }
...
export function createLayout(props: { overlaps?: string[], fullHeight?: boolean }) {
  const parentLayout = inject(ChaosLayoutKey, null)
  ...
  provide(ChaosLayoutKey, {
    register: (
      ...
      const instances = findChildrenWithProvide(ChaosLayoutItemKey, rootVm?.vnode)
      ...
      const layoutItemStyles = computed<CSSProperties>(() => {
        ...
        if (!item)
          throw new Error(`[Chaos] Could not find layout item "${id}"`)
