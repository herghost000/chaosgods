...
import { createChaosAdapter } from '@/locale/adapters/chaos'
...
export const LocaleSymbol: InjectionKey<LocaleInstance & RtlInstance> = Symbol.for('chaos:locale')
...
export function createLocale(options?: LocaleOptions & RtlOptions) {
  const i18n = options?.adapter && isLocaleInstance(options?.adapter) ? options?.adapter : createChaosAdapter(options)
...
export function useLocale() {
  ...
  if (!locale)
    throw new Error('[Chaos] Could not find injected locale instance')
...
export const RtlSymbol: InjectionKey<RtlInstance> = Symbol.for('chaos:rtl')
...
export function useRtl() {
  ...
  if (!locale)
    throw new Error('[Chaos] Could not find injected rtl instance')
