...
export const GoToSymbol: InjectionKey<GoToInstance> = Symbol.for('chaos:goto')
...
export function useGoTo(_options: Partial<GoToOptions> = {}) {
  ...
  if (!goToInstance)
    throw new Error('[Chaos] Could not find injected goto instance')

  const goTo = {
    ...goToInstance,
    // can be set via CLocaleProvider
