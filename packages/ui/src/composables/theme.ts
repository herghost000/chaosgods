...
export const ThemeSymbol: InjectionKey<ThemeInstance> = Symbol.for('chaos:theme')
...
function parseThemeOptions(options: ThemeOptions = genDefaults()): InternalThemeOptions {
  ...
  if (!options)
    return { ...defaults, isDisabled: true } as any
  ...
  function install(app: App) {
    ...
    if (head) {
      ...
      let styleEl = IN_BROWSER
        ? document.getElementById('chaos-theme-stylesheet')
        : null
...
export function provideTheme(props: { theme?: string }) {
  ...
  if (!theme)
    throw new Error('Could not find Chaos theme injection')
