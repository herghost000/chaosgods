...
import { IN_BROWSER, clamp, debounce, propsFactory } from '@/util'
...
export function useVirtual<T>(props: VirtualProps, items: Ref<readonly T[]>) {
  ...
  watchEffect(() => {
    itemHeight.value = Number.parseFloat(`${props.itemHeight || 0}`)
  })
  ...
  const last = shallowRef(Math.ceil(
    ...
    (Number.parseInt(`${props.height!}`) || display.height.value) / (itemHeight.value || 16),
  ) || 1)
  ...
  const viewportHeight = computed(() => {
    return containerRef.value === document.documentElement
      ...
      : contentRect.value?.height || Number.parseInt(`${props.height!}`) || 0
  })
